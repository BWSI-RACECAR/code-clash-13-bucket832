...
class Solution:
    def stonks(self, prices):
            max_profit = 0
            for buy1 in range(len(prices)):
                 for sell1 in range(buy1, len(prices)):
                      for buy2 in range(sell1, len(prices)):
                           for sell2 in range(buy2, len(prices)):
                                max_profit = max(max_profit, (prices[sell1] - prices[buy1]) + (prices[sell2]) - prices[buy2])
            return max_profit
